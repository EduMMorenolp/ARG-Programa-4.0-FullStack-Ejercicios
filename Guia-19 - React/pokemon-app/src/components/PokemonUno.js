...
function PokemonUno({ pokemonId, vida }) {
    const [pokemonData, setPokemonData] = useState(null);
    const [currentVida, setCurrentVida] = useState(vida);
    ...
    useEffect(() => {
        fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonId}`)
            ...
            .then(data => {
                setPokemonData(data);
                setCurrentVida(data.stats[0].base_stat);
            })
            .catch(error => {
    ...
    return (
        <div className={`pokemon-card ${pokemonData.types[0].type.name}`}>
            ...
            <ul>
                <li>
                    <strong>Vida:</strong>{currentVida}
                </li>
                <li>
